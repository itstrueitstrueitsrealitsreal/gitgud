...
import { z } from 'zod';
import { CreateMatchRequest, JoinMatchRequest, MatchResponse, ErrorResponse } from '../types';
...
export async function pvpRoutes(
  ...
  const getAuthenticatedUser = (request: FastifyRequest) => {
    ...
    return user as { githubId: string; username: string; accessToken: string; avatarUrl?: string };
  };

  // Create a new PVP match
  fastify.post<{ Body: CreateMatchRequest }>('/pvp/create', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post<{ Body: JoinMatchRequest }>('/pvp/join', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = getAuthenticatedUser(request);
      ...
      if (errorMessage.includes('Cannot join') || errorMessage.includes('already full')) {
        const errorResponse: ErrorResponse = {
          error: {
            code: 'BAD_REQUEST',
            message: errorMessage,
  ...
  // Get match status
  fastify.get('/pvp/match/:matchId', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  // Set player ready
  fastify.post('/pvp/ready/:matchId', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = getAuthenticatedUser(request);
      const matchId = (request.params as { matchId: string }).matchId;
        ...
        (async () => {
          try {
            ...
            const [signals1, signals2] = await Promise.all([
              (async () => {
                let signals = githubCache.get(match.player1!.username, maxRepos, false);
                if (!signals) {
                  signals = await githubService.getSignals(match.player1!.username, maxRepos, false);
                  githubCache.set(match.player1!.username, maxRepos, false, signals);
                }
                return signals;
              })(),
              (async () => {
                let signals = githubCache.get(match.player2!.username, maxRepos, false);
                if (!signals) {
                  signals = await githubService.getSignals(match.player2!.username, maxRepos, false);
                  githubCache.set(match.player2!.username, maxRepos, false, signals);
                }
                return signals;
              })(),
            ]);
            ...
            const [roast1, roast2] = await Promise.all([
              (async () => {
                let roast = openAICache.get(match.player1!.username, intensity);
                if (!roast) {
              ...
              (async () => {
                let roast = openAICache.get(match.player2!.username, intensity);
                if (!roast) {
            ...
            const verdict = await openAIService.compareUsers(
              { username: match.player1!.username, signals: signals1, roast: roast1 },
              { username: match.player2!.username, signals: signals2, roast: roast2 },
              language
            );
            ...
            if (verdict.winner !== 'tie') {
              const winner = verdict.winner === 'user1' ? match.player1!.username : match.player2!.username;
              const loser = verdict.winner === 'user1' ? match.player2!.username : match.player1!.username;
              leaderboardService.recordMatch(winner, loser);
            }
          } catch (error) {
            fastify.log.error({ matchId, error }, 'PVP match comparison failed');
          }
        })();
      }
  ...
  fastify.get('/pvp/my-match', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = getAuthenticatedUser(request);
