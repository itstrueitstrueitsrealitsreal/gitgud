...
const SESSION_SECRET = process.env.SESSION_SECRET || 'change-me-in-production-use-random-string';
const PORT = parseInt(process.env.PORT || '3000', 10);
// Default allowed origins for development
const DEFAULT_ORIGINS = ['http://localhost', 'http://localhost:80', 'http://localhost:5173', 'http://127.0.0.1', 'http://127.0.0.1:80', 'http://127.0.0.1:5173'];
const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS?.split(',').filter(Boolean) || DEFAULT_ORIGINS;
...
const githubToken = GITHUB_TOKEN && !GITHUB_TOKEN.includes('your_github') && GITHUB_TOKEN.length > 10
  ? GITHUB_TOKEN
  : undefined;
...
const sessionSecret = SESSION_SECRET.length >= 32 
  ? SESSION_SECRET 
...
if (GITHUB_CLIENT_ID && GITHUB_CLIENT_SECRET) {
  ...
  console.warn('⚠️  GitHub OAuth credentials not provided. PVP mode will not work.');
...
fastify.get('/', async () => {
  return {
    ...
    version: '2.0.0',
    description: 'Backend API for GitGud - GitHub Developer Comparison Platform',
    endpoints: {
...
fastify.register(async (fastify: FastifyInstance) => {
  await compareRoutes(fastify, githubService, openAIService, githubCache, openAICache, leaderboardService);
});
...
fastify.register(async (fastify: FastifyInstance) => {
  await pvpRoutes(fastify, pvpService, githubService, openAIService, githubCache, openAICache, leaderboardService);
});
